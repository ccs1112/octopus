/*
 * bar_test.c -- guest-side exerciser for the mxgpu-mini BARs.
 *
 ...
 *
 * Build (in the guest):  cc -O2 -Wall -o bar_test bar_test.c
 * Run:                   sudo ./bar_test <pci-bdf>   e.g. 0000:00:04.0
 *
...
static volatile void *map_bar(const char *bdf, int bar, size_t *len_out)
{
    ...
    snprintf(path, sizeof(path),
             "/sys/bus/pci/devices/%s/resource%d", bdf, bar);
    ...
    void *base = mmap(NULL, st.st_size, PROT_READ | PROT_WRITE,
                      MAP_SHARED, fd, 0);
    if (base == MAP_FAILED) {
        perror("mmap");
        exit(1);
...
/*
 * 32-bit accessors over a mapped BAR.
 *
 ...
 * "redundant" access, or reordering it past another -- mandatory here
 * because on BAR0 the "memory" is really your QEMU .read/.write handler,
 * and on the counter register two reads must produce two distinct bus
 * transactions (and two distinct values).
 */
